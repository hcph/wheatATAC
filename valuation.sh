...
do
bedtools closest -D ref -t all -mdb all -a /public/home/chaohe/db/geneR1.bed -b "$dir"/final/"$i".peaks.bed >"$i".txt
awk '{if($6=="-") print $1"\t"$4"\t"$6"\t"$10"\t"$17"\t"$3-$16-$8}' "$i".txt | uniq  >minus.txt
awk '{if($6=="+") print $1"\t"$4"\t"$6"\t"$10"\t"$17"\t"$16+$8-$2}' "$i".txt | uniq >plus.txt
